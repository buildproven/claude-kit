...
export function registerContentAnalysisTools(
  server: McpServer,
  ...
  registerTool(
    ...
    z.object({
      ...
      calculate_sentiment: z
        .boolean()
        .optional()
        .describe('Calculate sentiment'),
      calculate_toxicity: z.boolean().optional().describe('Calculate toxicity'),
      calculate_readability: z
        .boolean()
        .optional()
        .describe('Calculate readability'),
      calculate_keyword_density: z
        .boolean()
        .optional()
        .describe('Calculate keyword density'),
      calculate_information_score: z
        .boolean()
        .optional()
        .describe('Calculate information score'),
      calculate_adult_score: z
        .boolean()
        .optional()
        .describe('Calculate adult score'),
    }),
    async (params, client) => {
  ...
  registerTool(
    ...
    z.object({
      ...
      algo: z
        .enum(['percentile', 'linear', 'exponential'])
        .optional()
        .describe('Algorithm for distribution calculation'),
    }),
    async (params, client) => {
      const response = await client.post<DataForSeoResponse<any>>(
