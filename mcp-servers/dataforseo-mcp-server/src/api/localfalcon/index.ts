...
class LocalFalconClient {
  ...
  async get<T>(path: string, params?: Record<string, any>): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Local Falcon API error: ${response.status} ${response.statusText} - ${errorText}`
      )
    }
...
export function registerLocalFalconTools(
  server: McpServer,
  ...
  registerTool(
    ...
    z.object({
      ...
      distance: z
        .number()
        .describe('Distance between grid points (in miles or kilometers)'),
      units: z
      ...
      points: z
        ...
        .max(100)
        .default(20)
        .describe('Number of grid points to calculate'),
    }),
    async params => {
      const response = await client.post<any>('/v1/grid/', params)
  ...
  registerTool(
    ...
    z.object({
      query: z
        .string()
        .describe('Search query for finding Google My Business locations'),
      lat: z
        .number()
        .optional()
        .describe('Optional latitude to center search around'),
      lng: z
        .number()
        .optional()
        .describe('Optional longitude to center search around'),
      include_sabs: z
        .boolean()
        .default(true)
        .describe('Include Service Area Businesses in results'),
    }),
    async params => {
      const response = await client.post<any>('/v1/places/', params)
  ...
  registerTool(
    ...
    async params => {
      const response = await client.post<any>('/v1/result/', params)
  ...
  registerTool(
    ...
    async params => {
      const response = await client.post<any>('/v1/search/', params)
  ...
  registerTool(
    ...
    z.object({
      ...
      points: z
        ...
        .max(100)
        .default(20)
        .describe('Number of grid points to use'),
      language: z.string().optional().describe("Language code (e.g., 'en')"),
      country: z.string().optional().describe("Country code (e.g., 'us')"),
    }),
    async params => {
      const response = await client.post<any>('/v1/scan/', params)
