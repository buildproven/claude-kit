...
**Track API costs per feature, branch, and command. Understand ROI of AI-accelerated development.**
...
if [ "$MODE" = "branch" ]; then
  ...
  # ROI Analysis for feature branches:
  #   Development Cost vs Traditional (~$5K-10K) = 90-99% reduction
fi
```
...
if [ "$MODE" = "forecast" ]; then
  ...
  # ROI Context: Traditional dev $5K-10K/feature, AI-assisted $50-200/feature
fi
```
...
if [ "$MODE" = "expensive" ]; then
  ...
  # Tips: high ratio -> check model choice, repetitive -> cache/batch, expensive branches -> smaller features
fi
```
...
Every command using Claude API should track costs at the end:

```bash
if command -v ~/Projects/claude-setup/scripts/track-cost.sh &> /dev/null; then
  ~/Projects/claude-setup/scripts/track-cost.sh track \
    "$(basename "$0" .md)" "$MODEL" "$INPUT_TOKENS" "$OUTPUT_TOKENS" 2>/dev/null || true
fi
```
...
## Budget Configuration (CS-063)

Create `.claude/cost-config.json`:
...
- **monthlyBudget**: Target monthly spend in dollars (default: 200)
- **alertThresholds**: Percentages to trigger warnings (default: 75%, 90%, 100%)
- **blockAtLimit**: If true, blocks expensive operations at 100% budget (default: false)

## Model Cost Optimization (CS-080)
...
**Agent-specific:** Explore=Haiku, code-reviewer=Sonnet, architect-reviewer=Opus, security-auditor=Sonnet.
...
- `/bs:dashboard` - Unified observability with cost section
