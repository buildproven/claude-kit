...
default=$(gh repo view "$slug" --json defaultBranchRef --jq '.defaultBranchRef.name')
[ -n "$default" ] || { echo "audit-repo: cannot resolve default branch for $slug" >&2; exit 1; }
git -C "$REPO" rev-parse --verify "$default" >/dev/null
...
unmerged_branches=$(git -C "$REPO" for-each-ref --format='%(refname:short)' --no-merged "$default" refs/heads 2>/dev/null \
  | awk -v base_branch="$default" '$0 != base_branch {n++} END{print n+0}')
