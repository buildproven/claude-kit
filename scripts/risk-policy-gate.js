...
function matchesPattern(filepath, patterns) {
  return patterns.some(pattern => {
    ...
    // 1. Replace ** and * with placeholders before escaping
    ...
    // Escape regex special chars (not our placeholders)
    regexPattern = regexPattern.replace(/[\\^$+?.()|[\]{}]/g, '\\$&')

    // Restore glob patterns as regex
    regexPattern = regexPattern
      .replace(new RegExp(DOUBLE_STAR.replace(/\0/g, '\\0'), 'g'), '.*') // ** matches anything including /
      .replace(new RegExp(SINGLE_STAR.replace(/\0/g, '\\0'), 'g'), '[^/]*') // * matches anything except /
...
module.exports = { calculateRiskTier, validateRequiredChecks, matchesPattern }
