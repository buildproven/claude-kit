...
mkdir -p "$OUTPUT_DIR"
for evidence_name in execution-plan.json run-record.json provider; do
...
if [ -n "$EXECUTION_PLAN" ]; then
  ...
  GOVERNED_STARTED_AT_MS=$(python3 -c 'import time; print(time.time_ns() // 1000000)')
...
write_governed_record() {
  local status="$1" effective_provider="$2" exit_code="$3" failure_category="$4" finished_at_ms
...
if [ "$RC" -eq 0 ]; then
  ...
  write_governed_record passed "$PROVIDER" 0 ""
...
write_governed_record "$GOVERNED_STATUS" "$PROVIDER" "$RC" "$GOVERNED_FAILURE"
