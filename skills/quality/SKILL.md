...
```bash
QUALITY_SCRIPTS_DIR="$(for d in "${CLAUDE_PLUGIN_ROOT:-}" "${CLAUDE_KIT_ROOT:-}" "$HOME/.claude" .; do [ -n "$d" ] || continue; resolver="$d/scripts/quality-runtime-dir.sh"; [ -f "$resolver" ] || continue; bash "$resolver"; exit $?; done)"
