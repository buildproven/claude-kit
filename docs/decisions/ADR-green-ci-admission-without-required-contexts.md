...
mark a registered workflow as required, GitHub returns an empty required set
even when the exact PR head has a successful Actions check. The merger then
...
when that set is empty, inspect every registered PR check and classify the
exact head as already green when the set is non-empty and every check is
